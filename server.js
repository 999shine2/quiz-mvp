...
async function generateQuestionImage(question, userId, apiKey) {
    try {
        ...
        console.log(`[SEQ-V6] ⏩ START Generation for ${questionHash}: "${imagePrompt.substring(0, 40)}..."`);
        ...
        console.log(`[SEQ-V6] Using SiliconFlow key: ${siliconKey.substring(0, 5)}...${siliconKey.substring(siliconKey.length - 4)} (${siliconKey.length} chars)`);
        ...
        if (!imageBase64) {
            console.warn(`[SEQ-V6] Generation failed: API returned null`);
            return null;
        }
        ...
        if (imageBuffer.length < 1000) {
            console.warn(`[SEQ-V6] Generation failed (too small: ${imageBuffer.length} bytes)`);
            return null;
        }

        // Save to disk
        await fs.writeFile(filePath, imageBuffer);
        console.log(`[SEQ-V6] ✅ COMPLETE ${questionHash}: Saved ${filename} (${imageBuffer.length} bytes)`);

        return `/images/questions/${filename}`;
    } catch (error) {
        console.error('[SEQ-V6] Generation error:', error.message);
        return null;
    }
}
