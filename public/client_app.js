...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', async () => {
            ...
            if (username) {
                // Determine user language based on input (simple ASCII check)
                const isKorean = /[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/.test(username);
                localStorage.setItem('user_lang', isKorean ? 'ko' : 'en'); // Fix lang logic if needed

                currentUser = username;
                localStorage.setItem('study_user', currentUser);
                localStorage.setItem('user_name', currentUser); // SYNC FOR BUFFER LOGIC

                // Reload buffer for the new user immediately
                window.endlessBuffer = []; // Clear current
                await loadBufferFromLocal(); // Load new user's buffer

                checkAuth();

                // If buffer is empty after load, maintain will fill it.
                // If buffer restored 10 items, maintain will do nothing.
